...
Connection::Connection(Layer *layer, Layer *parent_layer, string properties, unordered_multimap<int, int> *time_connection) {
...
void Layer::Backward(int time_index) {
	...
	if (RNN) {
		...
		for (int k = 0; k < child_connection.size(); k++) {
			...
			for (auto s = connection->time_connection[1][t].begin(); s != connection->time_connection[1][t].end(); s++) {
				if (connection->properties[0] == 'P') {
					if (strstr(connection->properties.c_str(), "average")) {
						#pragma omp parallel for
						for (int h = 0; h < batch_size; h++) {
							float *error = &this->error[(h * time_step + t) * number_nodes];
							float *next_error = &child_layer->error[(h * child_layer->time_step + (*s)) * child_layer->number_nodes];
						...
						for (int h = 0; h < batch_size; h++) {
							...
							float *neuron = &this->neuron[(h * time_step + t) * number_nodes];
							float *next_error = &child_layer->error[(h * child_layer->time_step + (*s)) * child_layer->number_nodes];
							float *next_neuron = &child_layer->neuron[(h * child_layer->time_step + (*s)) * child_layer->number_nodes];
					...
					for (int h = 0; h < batch_size; h++) {
						...
						memcpy(error, next_error, sizeof(float) * number_nodes);
					}
				}
			}
...
void RNN::Activate(int time_index, bool training) {
	...
	if ((direction == 1 && t == time_step - 1) || (direction == -1 && t == 0)) {
	...
	for (int k = 0; k < layer->child_connection.size(); k++) {
		...
		if (connection->properties[0] == 'W' && strstr(connection->properties.c_str(), "recurrent")) {
			if ((direction == 1 && t < time_step - 1) || (direction == -1 && t > 0)) {
				...
				for (int h = 0; h < batch_size; h++) {
					float *error = &this->error[0][(h * time_step + t) * number_nodes];
					float *next_error = &this->error[1][(h * time_step + t + direction) * number_nodes];
		...
		if (strstr(connection->properties.c_str(), "copy") && ((direction == 1 && t == time_step - 1) || (direction == -1 && time_step == 0))) {
			for (int t = 0; t < time_step; t++) {
				if (connection->time_connection[1][t].size() > 0) {
					return;
				}
...
void RNN::Forward(int time_index) {
	...
	for (int k = 0; k < layer->connection.size(); k++) {
		...
		if (strstr(connection->properties.c_str(), "copy") && ((direction == 1 && t == 0) || (direction == -1 && t == time_step - 1))) {
			for (int t = 0; t < time_step; t++) {
				if (connection->time_connection[0][t].size() > 0) {
					return;
				}
