...
with tf.Session() as sess:
    ...
    for step in range(epochs):        
        ...
        for i in range(0, x_train.shape[0], batch_size):
            ...
            c, a, _ = sess.run([cost, accuracy, train], feed_dict={X: x_train[i:i+size], Y: y_train[i:i+size], keep_prob: 0.5})
            loss[0] += c * size
            score[0] += a * size
        ...
        for i in range(0, x_test.shape[0], batch_size):
            ...
            c, a = sess.run([cost, accuracy], feed_dict={X: x_test[i:i+size], Y: y_test[i:i+size], keep_prob: 1.0})
            loss[1] += c * size
            score[1] += a * size
