...
void Batch_Normalization::Set_Optimizer(Optimizer *optimizer) {
	delete gamma_optimizer;
	gamma_optimizer = optimizer->Copy();
	gamma_optimizer->Resize_Memory(number_maps);

	delete beta_optimizer;
	beta_optimizer = optimizer->Copy();
	beta_optimizer->Resize_Memory(number_maps);
}
...
void Connection::Set_Optimizer(Optimizer *optimizer) {
	if (weight) {
		...
		this->optimizer->Resize_Memory(number_weights);
	}
	if (LSTM_weight) {
		for (int h = 0; h < LSTM_weight->number_weight_types; h++) {
			delete LSTM_weight->optimizer[h];
			LSTM_weight->optimizer[h] = optimizer->Copy();
			LSTM_weight->optimizer[h]->Resize_Memory(number_weights);
		}
	}
}
...
double Connectionist_Temporal_Classification::Backward_Algorithm(vector<string> label_sequence, int length_event, double **beta, float _likelihood[]) {
	...
	for (int t = length_event - 1, number_labels = label_sequence.size(); t >= 0; t--) {
		...
		for (int i = 0; i < label_sequence.size(); i++) {
			beta[t][i] *= scale;
		}
	}
...
void Layer::Construct() {
	...
	if (strstr(properties.c_str(), "LSTM")) {
		...
		if (strstr(properties.c_str(), "PReLU")) {
...
void Layer::Set_Optimizer(Optimizer *optimizer) {
	...
	if (bias_optimizer) {
		delete bias_optimizer;
		bias_optimizer = optimizer->Copy();
		bias_optimizer->Resize_Memory(number_maps);
	}
	if (slope_optimizer) {
		delete slope_optimizer;
		slope_optimizer = optimizer->Copy();
	...
	if (LSTM_node) {
		...
		for (int h = 0; h < LSTM_node->number_weight_types; h++) {
			...
			LSTM_node->bias_optimizer[h] = optimizer->Copy();
			LSTM_node->bias_optimizer[h]->Resize_Memory(number_maps);
		}
		for (int h = 0; h < LSTM_node->number_weight_types - 1; h++) {
			delete LSTM_node->peephole_optimizer[h];
			LSTM_node->peephole_optimizer[h] = optimizer->Copy();
			LSTM_node->peephole_optimizer[h]->Resize_Memory(number_maps);
		}
	}
	delete optimizer;
}
	...
	if (strstr(layer->properties.c_str(), "LSTM")) {
		...
		for (int j = 0; j < layer->number_nodes; j++) {
			...
			for (int h = 0, k = j / layer->map_size; h < batch_size; h++) {
				...
				else if (reverse == true && t + 1 < time_step) {
					input_neuron[0][index] += LSTM_node->peephole[LSTM_node->input][k] * previous_cell_output_neuron[index + layer->number_nodes];
					forget_neuron[0][index] += LSTM_node->peephole[LSTM_node->forget][k] * previous_cell_output_neuron[index + layer->number_nodes];
					output_neuron[0][index] += LSTM_node->peephole[LSTM_node->output][k] * previous_cell_output_neuron[index + layer->number_nodes];
				}
		...
		if (strstr(layer->properties.c_str(), "ELU")) {
			...
			for (int j = 0; j < layer->number_nodes; j++) {
				for (int h = 0; h < batch_size; h++) {
					...
					layer->neuron[0][index] = (layer->neuron[0][index] > 0) ? (layer->neuron[0][index]) : (parameter * (exp(layer->neuron[0][index]) - 1));
				}
			}
		}
			...
			for (int j = 0; j < layer->number_nodes; j++) {
				for (int h = 0; h < batch_size; h++) {
					...
					layer->neuron[0][index] *= (layer->neuron[0][index] > 0) ? (1) : (slope);
				}
			}
		}
						...
						if (layer->LSTM_node) {
							...
							for (int i = 0; i < LSTM_weight->number_weight_types; i++) {
								for (int h = 0; h < batch_size; h++) {
									...
									parent_layer->error[0][((reverse == false) ? (h * time_step + t - 1) : (h * time_step + t + 1)) * parent_layer->number_nodes + j] += sum;
								}
							}
						}
						else {
				...
				if (connection->properties[0] == 'P') {
					...
					if (recurrent == false) {
						...
						if (layer->LSTM_node) {
							...
							for (int i = 0; i < LSTM_weight->number_weight_types; i++) {
								for (int h = 0; h < batch_size; h++) {
									float sum = 0, *neuron = &parent_layer->neuron[0][((reverse == false) ? (h * time_step + t - 1) : (h * time_step + t + 1)) * parent_layer->number_nodes];
						...
						for (int t = 0; t < time_step; t++) {
							if (recurrent == false){
								...
								for (int h = 0; h < batch_size; h++) {
									...
									float *neuron = &parent_layer->neuron[0][((reverse == false) ? (h * time_step + t - 1) : (h * time_step + t + 1)) * parent_layer->number_nodes];
		...
		for (int j = 0; j < layer->number_maps; j++) {
			...
			for (int i = 0; i < LSTM_node->number_weight_types - 1; i++) {
				...
				for (int h = 0; h < batch_size * time_step; h++) {
					...
					else if (reverse == true && (h % time_step) + 1 < time_step) {
						for (int k = 0; k < layer->map_size; k++) {
							sum += ((batch_normalization) ? (LSTM_node->batch_normalization[i][0]->error_backup[index + k]) : (LSTM_node->error[i][0][index + k])) * previous_cell_output_neuron[index + k + layer->number_nodes];
						}
					}
	...
	if (strstr(layer->properties.c_str(), "LSTM")) {
		...
		for (int j = 0; j < layer->number_nodes; j++) {
			...
			for (int h = 0, k = j / layer->map_size; h < batch_size; h++) {
				...
				if (reverse == false && t + 1 < time_step) {
		...
		if (strstr(layer->properties.c_str(), "ELU")) {
			...
			for (int j = 0; j < layer->number_nodes; j++) {
				for (int h = 0; h < batch_size; h++) {
					...
					layer->error[0][index] *= (layer->neuron[0][index] > 0) ? (1) : (layer->neuron[0][index] + parameter);
				}
			}
		}
		else if (strstr(layer->properties.c_str(), "PReLU")) {
			...
			for (int j = 0; j < layer->number_nodes; j++) {
				for (int h = 0; h < batch_size; h++) {
					...
					layer->error[0][index] *= (layer->neuron[0][index] > 0) ? (1) : (slope);
				}
			}
		}
...
double Neural_Networks::Train(int batch_size, int number_training, int length_data[], float ***input, float ***target_output, vector<string> target_label_sequence[], double learning_rate, double epsilon, double noise_standard_deviation) {
	...
	// set epsilon for batch normalization
	for (int i = 0; i < layer_depth; i++) {
		for (int j = 0; j < layer[i].size(); j++) {
...
float Optimizer::Calculate_Gradient(int parameter_index, double _gradient, double learning_rate) {
	...
	if (type == 2) { // nesterov
		float v = factor_1 * velocity - learning_rate * _gradient;

		return (gradient = (velocity = factor_1 * v - learning_rate * _gradient));
	}
	if (type == 3) { // adagrad
		float c = velocity + _gradient * _gradient;

		return (gradient = -learning_rate * _gradient / sqrt((velocity = c) + epsilon));
	}
	if (type == 4) { // rmsprop
		float c = factor_1 * velocity + (1 - factor_1) * _gradient * _gradient;
	...
	if (type == 6) { // adam
		float m = factor_1 * momentum + (1 - factor_1) * _gradient;
		float v = factor_2 * velocity + (1 - factor_2) * _gradient * _gradient;
...
Optimizer* Optimizer::Copy() {
	...
	return new Optimizer(name[type], epsilon, factor_1, factor_2);
}