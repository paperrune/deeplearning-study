...
void Multilayer_Perceptron::Activate(char option[], int layer_index, int node_index) {
	int i = layer_index;
	int j = node_index;
...
void Multilayer_Perceptron::Adjust_Parameter(int layer_index, int node_index) {
	int i = layer_index;
	int j = node_index;
	...
	for (int h = 0; h < batch_size; h++) {
		...
		for (int k = 0; k < number_nodes[i - 1]; k++) {
			weight[i][j][k] -= derivative[j] * lower_neuron[k];
		}
		weight[i][j][number_nodes[i - 1]] -= derivative[j];
	}
}
void Multilayer_Perceptron::Backpropagate(int layer_index, int node_index) {
...
void Multilayer_Perceptron::Differentiate(int layer_index, int node_index, double learning_rate, double **target_output) {
	int i = layer_index;
	int j = node_index;
...
void Multilayer_Perceptron::Feedforward(int layer_index, int node_index) {
	int i = layer_index;
	int j = node_index;
	...
	for (int h = 0; h < batch_size; h++) {
		...
		neuron[j] = sum + weight[i][j][number_nodes[i - 1]];
	}
}
...
void Multilayer_Perceptron::Batch_Normalization_Activate(char option[], int layer_index, int node_index) {
	int i = layer_index;
	int j = node_index;
...
void Multilayer_Perceptron::Batch_Normalization_Adjust_Parameter(int layer_index, int node_index) {
	int i = layer_index;
	int j = node_index;
...
void Multilayer_Perceptron::Batch_Normalization_Differentiate(int layer_index, int node_index) {
	int i = layer_index;
	int j = node_index;
...
void Multilayer_Perceptron::Construct_Networks() {
	...
	for (int i = 1; i < number_layers; i++) {
		if (strstr(this->type_layer[i], "bn")) {
			gamma[i] = new double[number_nodes[i]];
			beta[i] = new double[number_nodes[i]];
			mean[i] = new double[number_nodes[i]];
			variance[i] = new double[number_nodes[i]];
			sum_mean[i] = new double[number_nodes[i]];
			sum_variance[i] = new double[number_nodes[i]];
		}

		weight[i] = new double*[number_nodes[i]];

		for (int j = 0; j < number_nodes[i]; j++) {
			weight[i][j] = new double[number_nodes[i - 1] + 1];
		}
	}
...
Multilayer_Perceptron::Multilayer_Perceptron(string path) {
	...
	if (file.is_open()) {
		...
		for (int i = 1; i < number_layers; i++) {
			if (strstr(this->type_layer[i], "bn")) {
				for (int j = 0; j < number_nodes[i]; j++) file >> gamma[i][j];
				for (int j = 0; j < number_nodes[i]; j++) file >> beta[i][j];
				for (int j = 0; j < number_nodes[i]; j++) file >> mean[i][j];
				for (int j = 0; j < number_nodes[i]; j++) file >> variance[i][j];
			}
			for (int j = 0; j < number_nodes[i]; j++) {
				for (int k = 0; k < number_nodes[i - 1] + 1; k++) {
					file >> weight[i][j][k];
				}
			}
		}
...
Multilayer_Perceptron::Multilayer_Perceptron(string type_layer[], int number_layers, int number_nodes[]) {
	this->number_layers = number_layers;
	this->number_nodes = new int[number_layers];
	this->type_layer = new char*[number_layers];

	for (int i = 0; i < number_layers; i++) {
		strcpy(this->type_layer[i] = new char[type_layer[i].size() + 1], type_layer[i].c_str());
		this->number_nodes[i] = number_nodes[i];
	}
	Construct_Networks();
}
...
void Multilayer_Perceptron::Save_Model(string path) {
	...
	for (int i = 0; i < number_layers; i++) file << number_nodes[i] << endl;
	file << epsilon << endl;

	for (int i = 1; i < number_layers; i++) {
		if (strstr(type_layer[i], "bn")) {
			for (int j = 0; j < number_nodes[i]; j++) file << gamma[i][j] << endl;
			for (int j = 0; j < number_nodes[i]; j++) file << beta[i][j] << endl;
			for (int j = 0; j < number_nodes[i]; j++) file << mean[i][j] << endl;
			for (int j = 0; j < number_nodes[i]; j++) file << variance[i][j] << endl;
		}
...
void Multilayer_Perceptron::Test(int batch_size, double **input, double **output) {
	...
	#pragma omp parallel for
	for (int h = 0; h < batch_size; h++) {
		memcpy(neuron[0][0][h], input[h], sizeof(double) * number_nodes[0]);
	}
	for (int i = 1; i < number_layers; i++) {
		#pragma omp parallel for
		for (int j = 0; j < number_nodes[i]; j++) {
	...
	#pragma omp parallel for
	for (int h = 0; h < batch_size; h++) {
		memcpy(output[h], neuron[0][number_layers - 1][h], sizeof(double) * number_nodes[number_layers - 1]);
	}
}
...
double Multilayer_Perceptron::Train(int batch_size, int number_training, double epsilon, double learning_rate, double **input, double **target_output) {
	...
	for (int i = 0; i < number_layers; i++) {
		if (strstr(type_layer[i], "bn")) {
			memset(sum_mean[i], 0, sizeof(double) * number_nodes[i]);
			memset(sum_variance[i], 0, sizeof(double) * number_nodes[i]);
		}
	}
	...
	for (int g = 0, h = 0; g < number_training; g++) {
		...
		if (++h == batch_size) {
			...
			for (int h = 0; h < batch_size; h++) {
				for (int i = number_layers - 1, j = 0; j < number_nodes[i]; j++) {
					if (strstr(type_layer[i], "ce")) {
...
double Multilayer_Perceptron::Train(int batch_size, int number_training, int length_data[], double epsilon, double learning_rate, double ***input, double ***target_output) {
	...
	for (int g = 0, h = 0; g < number_training; g++) {
		for (int s = 0; s < length_data[g]; s++) {
			memcpy(neuron[0][0][h], input[index[g][s][0]][index[g][s][1]], sizeof(double) * number_nodes[0]);
